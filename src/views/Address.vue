  ...
  <div class="address-ctn">
      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(item, index) in displayAddressList" 
                    :class="{check: (orderAddrId === item.addressId || (orderAddrId === '' &&  item.isDefault))}"
                    @click="checkCurAddr(item.addressId)">
                  <dl>
                    <dt>{{item.userName}}</dt>
                  ...
                  <div class="addr-opration addr-del">
                    <a href="javascript:;" class="addr-del-btn" @click="confirmDeleteAddr(item.addressId)">
                      <i class="iconfont iconshanchu" style="font-size: 20px;"></i>
                    </a>
                  </div>
                  <div v-show="!item.isDefault" class="addr-opration addr-set-default">
                    <a href="javascript:;" class="addr-set-default-btn" @click="setDefaultAddr(item.addressId)"><i>Set default</i></a>
                  </div>
                  <div v-show="item.isDefault" class="addr-opration addr-default">Default address</div>
                </li>
                <li class="addr-new">
            ...
            <div v-show="addressList.length > minDisplayNum" class="shipping-addr-more">
              <a class="addr-more-btn more-or-fold-btn up-down-btn" href="javascript:;"
                 :class="[addressList.length > displayAddressNum? 'more' : 'fold']"
                 @click="expandOrFoldAddress">
                <span class="more-txt txt">更多</span>
                <span class="fold-txt txt">折叠</span>
                <i class="i-up-down arrow">
          ...
          <div class="next-btn-wrap">
            <a href="javascript:;" @click="confirmOrder" class="btn btn--m btn--red">下一步</a>
          </div>
        </div>
      </div>
    </div>
    ...
    <modal v-model="showDelConfirm">
      ...
      <template #button>
        ...
        <a class="btn btn--m btn--red" href="javascript:;" @click="showDelConfirm=false;">取消</a>
      </template>
    </modal>
    <modal v-model="showErrConfirm">
      <template #content>
        <p>{{errTip}}</p>
      </template>
      <template #button>
        <a href="javascript:;" class="btn btn--m btn--red"
        @click="showErrConfirm = false;">好的</a>
      </template>
    </modal>
  </div>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        showErrConfirm: false,
        errTip: '',
        // addrCheckedIdx: 0,
        orderAddrId: '',
        minDisplayNum: 3,
        displayAddressNum: 3 //默认展示3条数据
      }
    },
    computed: {
      displayAddressList() {
        return this.addressList.slice(0, this.displayAddressNum);
      }
    },
    methods: {
      async getAddressList(init) {
        let {code, data} = await API.address.getAddressList();
        if (code === 0) {
          this.addressList = data;
          if (init) {
            this.addressList.forEach(item => {
              if (item.isDefault) {
                this.orderAddrId = item.addressId;
              }
            });
          }
        }
      ...
      expandOrFoldAddress() {
        if (this.displayAddressNum < this.addressList.length) {
          this.displayAddressNum = this.addressList.length;
        } else {
          this.displayAddressNum = this.minDisplayNum;
        }
      },
      checkCurAddr(addrId) {
        // this.addrCheckedIdx = checkedIdx;
        this.orderAddrId = addrId;
      },
      async setDefaultAddr(addrId) {
        let {code, data} = await API.address.setDefault({addrId});
        if (code === 0) {
          this.addressList = data.addressList;
        }
      },
      confirmDeleteAddr(addressId) {
        this.delAddressId = addressId;
        this.showDelConfirm = true;
      },
      reloadAddresList() {
        this.addressList = this.addressList.filter(item => {
          return item.addressId !== this.delAddressId;
        });
      },
      async delAddress() {
        if (this.addressList.length === 1) {
          this.errTip = '地址列表至少有一条数据，已无法继续删除。';
          this.showErrConfirm = true;
          return;
        }
        let addressId = this.delAddressId;
        let {code, data} = await API.address.delete({addressId});
        if (code === 0) {
          if (addressId === this.orderAddrId) {
            this.orderAddrId = '';
          }
          this.reloadAddresList();
        }
        this.showDelConfirm = false;
        this.delAddressId = '';
      },
      confirmOrder() {
        let orderAddrId = this.orderAddrId;
        if (orderAddrId === '') {
          this.errTip = '请选择您的配送地址';
          this.showErrConfirm = true;
          return;
        }
        this.$router.push({name: 'orderConfirm', params: {addressId: orderAddrId}})
      }
    },
    mounted() {
