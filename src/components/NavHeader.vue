<template>
    <header class="header">
      ...
      <div class="navbar">
        ...
        <div class="navbar-right-contaiiner" style="display: flex;">
          <div class="navbar-menu-container">
            <i style="margin-right: -12px; font-size: 18px;" v-if="nickName" class="iconfont iconyonghu"></i>
            <span class="navbar-link" v-text="nickName" v-if="nickName"></span>
            <a href="javascript:void(0)" class="navbar-link" v-if="!nickName" @click="showLoginModal">Login</a>
            <a href="javascript:void(0)" class="navbar-link" v-else @click="logout">Logout</a>
            <div class="navbar-cart-container">
              <span class="navbar-cart-count" v-if="cartCount>0">{{cartCount}}</span>
              <a class="navbar-link navbar-cart-link" href="/#/cart">
                <i style="font-size: 25px;" class="iconfont icongouwuche"></i>
    ...
    methods: {
      ...
      async login() {
         ...
         if (userName === '' || password === '') {
           this.errTip = true;
           this.errTipText = '请输入用户名或密码';
           return;
         }
        let rs = await API.users.login({
          userName,
          password
        });
        if (rs.code === 0) {
